...
var (
	MCPURL      string // MCP Server SSE 地址
	OllamaURL   string // Ollama 服务地址
	OllamaModel string // Ollama 模型名
	GatewayPort string // Gateway 监听端口
)

// loadConfig 从环境变量加载配置，未设置时用默认值兜底
func loadConfig() {
	MCPURL = getEnv("MCP_SERVER_URL",
		"http://mcp-server-service.ai-services.svc.cluster.local:8080/sse")
	OllamaURL = getEnv("OLLAMA_URL",
		"http://ollama-service.ai-services.svc.cluster.local:11434")
	OllamaModel = getEnv("OLLAMA_MODEL", "qwen3.5:4b")
	GatewayPort = getEnv("GATEWAY_PORT", "3000")

	log.Printf("Config loaded: MCP=%s Ollama=%s Model=%s Port=%s",
		MCPURL, OllamaURL, OllamaModel, GatewayPort)
}

// getEnv 读取环境变量，为空返回默认值
...
func handleHome(w http.ResponseWriter, r *http.Request) {
	_, err := fmt.Fprint(w, htmlHead)
	if err != nil {
		log.Printf("fmt.Fprint(w, htmlHead), err: %v", err.Error())
	}
	body := `
        <h2>HR 智能助手 (基于 MCP + Qwen)</h2>
        <form action="/chat" method="POST">
            <textarea name="prompt" rows="5" placeholder="例如：查询销售部门的所有员工信息..."></textarea><br>
            <input type="submit" value="发送查询">
        </form>
    </body></html>`
	_, err2 := fmt.Fprint(w, body)
	if err2 != nil {
		log.Printf("fmt.Fprint(w, body), err: %v", err.Error())
	}
}
...
func callOllama(msgs []Message, tools []OllamaTool) (*OllamaResponse, error) {
	req := OllamaRequest{
		Model:    OllamaModel, // qwen2.5:7b，有很好的工具调用功能以及中文处理和SQL书写能力
		Messages: msgs,
		Stream:   false,
		Tools:    tools,
	}
	...
	resp, err := http.Post(OllamaURL+"/api/chat", "application/json",
		strings.NewReader(string(jsonData))) // <-- 从全局变量读
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
